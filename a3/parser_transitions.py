...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        if len(unfinished_parses) < batch_size:
            batch_size = len(unfinished_parses)
        mini_unfinished_parses = unfinished_parses[:batch_size].copy()
        transitions = model.predict(mini_unfinished_parses)
        for i,little_one in enumerate(mini_unfinished_parses):
            little_one.parse_step(transitions[i])
            if (len(little_one.buffer) == 0) & (len(little_one.stack) == 1):
                unfinished_parses.remove(little_one)
    for i in range(len(sentences)):
